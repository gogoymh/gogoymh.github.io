...
## 방법
* 최근 pretrained model을 이용하여 놀라운 퍼포먼스를 보여주는 연구가 많은 가운데 저자들은 조금 다른 방식으로 접근한다.\\
  가령 Vision Transformer가 ImageNet 21K에서 학습한 뒤, ImageNet 1K로 downstream하는 방식의 finetuning을 하는데,\\
  이는 pretraining 과정에 이미지 21K장의 방대한 양을 학습해야하기 때문에 상당한 비용이 요구된다.\\
  논문은 <span class="highlight-yellow">21K장의 이미지를 이용하지 않고도 21K장을 이용한 만큼 혹은 그 이상의 퍼포먼스를 보여줄 수 있을지</span>에 대한 방법을 제시한다.
