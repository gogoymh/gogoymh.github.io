...
* 즉, <span class="highlight-yellow">DINO는 Teacher model의 output을 마치 ground truth처럼 생각하고,\\
  ...
  프레임워크의 이름 DINO는 Self-$\textbf{di}$stillation with $\textbf{no}$ labels에서 따온 것이다.
* 안정적인 학습을 위해 centering과 sharpening이라는 테크닉을 추가적으로 사용하며,\\
  다른 Self-supervised learning 방식에 비해 직관적이고, batch size가 크지 않아도 학습이 잘된다.
