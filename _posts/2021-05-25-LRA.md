...
3. BYTE-LEVEL DOCUMENT RETRIEVAL\\
  ...
  데이터는 input에 두 개의 논문이 들어가고, output은 두 논문이 인용이 되어있다면 1, 관련이 없다면 0으로 이진 분류 문제다.\\
...
* 6가지 태스크에 대하여 기본 트랜스포머와 variants 10개로 실험했다.\\
  다만 각 모델의 저자가 직접 실험한 것이 아니기 때문에 대략적인 결과라는 것을 강조했다.\\
  6가지 태스크 모두 분류 문제이기 때문에 Metric은 accuracy이며,\\
  각 태스크에 대해 가장 높은 값은 **볼드체**로, 두번 째로 높은 값은 <ins>밑줄</ins>로 표시했다.
