...
3. BYTE-LEVEL DOCUMENT RETRIEVAL\\
  두 문서가 얼마나 유사한지 모델링하는 문제다.\\
  즉, 문서를 byte/character-level로 문장을 임베딩하여 긴 시퀀스를 만들고,\\
  긴 시퀀스를 압축한 representation간의 similarity를 비교하여 매칭하는 테스크다.\\
  데이터는 input에 두 개의 논문이 들어가고, output은 두 논문이 서로 인용이 되어있다면 1, 관련이 없다면 0으로 이진 분류하는 문제다.\\
