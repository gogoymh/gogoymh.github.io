...
* 두 모델의 output은 마지막으로 softmax layer를 통과하고,\\
  ...
  그리고 Student model의 output과 cross entropy loss를 만들어 gradient descent해준다.\\
...
* 즉, <span class="highlight-yellow">DINO는 Teacher model의 output을 마치 ground truth처럼 생각하고,\\
  Student model의 output과 cross entropy를 구해 학습한다.</span>\\
  학습된 모델에서 정보를 추출(distillation)하는 방식을 이용하는 방식의 self-supervised learning이다.\\
