...
* 예를 들면, ImageNet 1K에서 pretrain된 ResNet50을 teacher model로 하여,\\
  ImageNet 22K에서 SIFT를 이용해 ImageNet 1K의 validation set과 비슷한 이미지를 제외하고,\\
  나머지 이미지 중 각 클래스별 score가 높은 4000개를 선별하여 4M개만 사용한다.
* 작성중
