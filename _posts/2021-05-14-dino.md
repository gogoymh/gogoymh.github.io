...
* 안정적인 학습을 위해 centering과 sharpening이라는 테크닉을 추가했고,\\
  다른 Self-supervised learning(SSL) 방식에 비해 직관적이고, batch size가 커야 학습 효과가 좋은 contrastive learning 방식보다 효율적이다.
...
* $k$-NN을 통해 좋은 퍼포먼스를 내기 위한 3가지 요소를 설명했다.\\
  MoCo 논문에서 제시한 momentum encoder, SwAV 논문에서 제시한 multi-crop,\\
  그리고 본 논문에서 발견한 사실: ViT를 사용할 때 작은 크기의 patch의 중요성 3가지다.
  DINO를 학습할 때는 이 3가지 요소를 모두 반영하여 학습했다.
...
* 이 논문에서 또 하나 강조한 것는 ViT를 SSL 했을 때\\
  <span class="highlight-red">마지막 block의 self-attention map이 위 그림처럼 semantic segmentation에 대한 explicit한 정보를 담고 있다는 것이다.</span>\\
  이는 supervision으로 학습한 ViT의 attention map에는 explicit하게 나타나지 않던 특징이, SSL을 통해 배웠다는 것이다.\\
  Self-attention의 각 head를 하나의 색깔(클래스)로 나타내면 위 그림과 같이 실제 segmentation 처럼 visualize 할 수 있다.
